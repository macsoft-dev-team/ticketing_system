...
export const useConversation = (ticketId) => {
  ...
  const formatMessage = useCallback((rawMessage) => {
    return {
      id: rawMessage.id,
      message: rawMessage.content || rawMessage.message || '',
      isOwnMessage: rawMessage.senderId === user?.id,
      timestamp: moment(rawMessage.createdAt || rawMessage.timestamp).format('hh:mm A'),
      senderName: rawMessage.sender?.name || (rawMessage.senderId === user?.id ? 'You' : 'User'),
      avatar: rawMessage.sender?.name?.charAt(0) || (rawMessage.senderId === user?.id ? user?.name?.charAt(0) || 'A' : 'U'),
  ...
  const fetchMessages = useCallback(async () => {
    if (!ticketId || !token) {
      console.log('Cannot fetch messages - missing ticketId or token:', { ticketId, token: !!token });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          errorMessage = 'Access denied. You may not have permission to view this conversation.';
  ...
  // Send message
  const sendMessage = useCallback(async (messageContent, attachments = []) => {
    if (!ticketId || !token || (!messageContent.trim() && attachments.length === 0)) {
    ...
    console.log('Sending message:', { messageContent, attachments: attachments.length });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          errorMessage = 'Access denied. You may not have permission to send messages.';
        } else if (response.status === 404) {
          errorMessage = 'Ticket not found or conversation endpoint not available.';
        } else if (response.status >= 500) {
          errorMessage = 'Server error. Please try again later.';
        }
  ...
  const handleNewMessage = useCallback((newMessage) => {
    ...
    try {
      ...
      setMessages(prevMessages => {
        ...
        return updatedMessages.sort((a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0));
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log(`🏠 Joined conversation room for ticket: ${ticketId}`);
    ...
    return () => {
      ...
      if (socket && socket.connected) {
        ...
        console.log(`👋 Left conversation room for ticket: ${ticketId}`);
      }
    };
  ...
  const testConnection = useCallback(async () => {
    try {
      ...
      console.log('API base connection test:', response.status, response.statusText);
      return response.ok;
    } catch (error) {
