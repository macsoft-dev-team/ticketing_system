...
export const hashSnaDevicePasswords = async (
  devices,
  onProgress,
  saltRounds = 10,
  shouldCancel = () => false
  ...
  for (let i = 0; i < totalDevices; i++) {
    ...
    try {
      ...
      // Hash the password field if it exists
      if (
        device.snamqttpassword &&
        typeof device.snamqttpassword === "string"
      ) {
        deviceCopy.snamqttpassword = await bcrypt.hash(
          device.snamqttpassword,
          saltRounds
        );
        // Optionally remove the original password for security
        delete deviceCopy.snamqttpassword;
      } else if (device.imeinumber) {
        // If no password field, use IMEI as password (as per existing logic)
        deviceCopy.snamqttpassword = await bcrypt.hash(
          device.imeinumber.toString(),
          saltRounds
        );
      }
      ...
      console.error(
        `Error hashing password for device ${device.imeinumber || i}:`,
        error
      );
      // Still add the device but without hashed password
      hashedDevices.push({ ...device, hashError: error.message });
