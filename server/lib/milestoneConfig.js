...
const milestoneStageConfig = [
  ...
  },
  {
    ...
    notes: '4 specific photos required: Controller Front, Controller Bottom, Full View Open, MCB Close Up',
  },
  {
...
function getNextAvailableStages(currentStage, userRole) {
  ...
  const allowedTransitions = {
    ...
    REPLACEMENT_IN_PROGRESS: ["SPARE_REQUESTED", "REQUEST_CLEARED_AT_FIELD", "REPAIRED"],
  ...
  const nextStages = nextStageNames
    ...
    .filter(config => config && canRoleTransitionToStage(userRole, config.stage));

  return nextStages;
}
 
function validateMilestoneTransition(userRole, targetStage, currentMilestone, data = {}) {
  ...
  if (currentMilestone) {
    ...
    if (currentMilestone) {
      ...
      const allowedTransitions = {
        TICKET_RAISED: ['REQUEST_CLEARED_AT_FIELD', 'SERVICE_CENTER_ASSIGNED'],
        REQUEST_CLEARED_AT_FIELD: [], // Final stage
        SERVICE_CENTER_ASSIGNED: ['REQUEST_CLEARED_AT_FIELD', 'SENT_TO_SERVICE_CENTER'],
        SENT_TO_SERVICE_CENTER: ['RECEIVED_AT_SERVICE_CENTER'],
        RECEIVED_AT_SERVICE_CENTER: ['DIAGNOSIS_IN_PROGRESS'],
        DIAGNOSIS_IN_PROGRESS: ['REPAIR_IN_PROGRESS', 'REPLACEMENT_IN_PROGRESS'],
        SPARE_REQUESTED: ['SPARE_APPROVED'],
      ...
      if (!allowed.includes(targetStage)) {
        return {
          valid: false,
          error: `Invalid transition from ${currentConfig?.label || currentStage} to ${targetConfig.label}`,
        };
      }
    }
