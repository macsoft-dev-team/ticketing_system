...
async function migrateExistingTickets() {
  ...
  try {
    ...
    if (!sequence) {
      ...
      console.log(
        `✅ Created sequence for ${currentYear} starting at ${existingTicketsThisYear}`
      );
    }
    ...
    if (totalTickets > 0) {
      ...
      console.log(
        "ℹ️ If any existing tickets need ticket code updates, please run a separate data fix"
      );
    } else {
    ...
    for (const seq of allSequences) {
      console.log(
        `  ${seq.year}: ${seq.prefix}-${seq.year}-${String(
          seq.lastNumber
        ).padStart(3, "0")} (${seq.lastNumber} tickets)`
      );
    }
