...
export const createMessage = createAsyncThunk(
    ...
    async ({ ticketId, message }, { rejectWithValue }) => {
        try {
            const response = await axios.post(`${API_URL}/conversation/${ticketId}`, message);
            return { data: response.data };
        } catch (error) {
            return rejectWithValue({ message: error.message });
        }
...
export const { setCurrentMessage, setShowConversation,setConversation } = conversationSlice.actions;
